...
class Constants(BaseConstants):
    ...
    # Treatment 2: J with C in last 3 rounds (experimental)
    partner_sequence_experimental = (
            ['J', 'C', 'R'] +  # Rounds 1-3: some random letters
            ['J'] * 9 +  # Rounds 4-12: J
            ['C'] * 3  # Rounds 13-15: different letter (last 3 rounds)
    )
...
class Player(BasePlayer):
    ...
    def calculate_points(self):
        """Calculate points for this round and update totals"""
        partner_choice = self.get_partner_choice()
        matched = self.letter_choice == partner_choice
        
        if matched:
            self.points_earned = 10
            # Update total points in participant vars
            current_total = self.participant.vars.get('total_points', 0)
            self.participant.vars['total_points'] = current_total + 10
        else:
            self.points_earned = 0
        
        return matched, self.points_earned
...
class Results(Page):
    ...
    def vars_for_template(player):
        ...
        matched, points_earned = player.calculate_points()
...
class FinalResults(Page):
    ...
    def vars_for_template(player):
        ...
        for round_num in range(1, Constants.num_rounds + 1):
            ...
            # Player 2: The one who chose 'C' in last 3 rounds (if experimental treatment)
            if treatment == 'experimental' and round_num >= (Constants.num_rounds - 2):  # Last 3 rounds
                player2_choice = 'C'
            elif round_num == 1:
                player2_choice = 'J'
            elif round_num == 2:
