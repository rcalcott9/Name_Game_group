...
class Player(BasePlayer):
    ...
    treatment = models.StringField()  # 'no_info' or 'info'

    def get_randomized_letter_choices(self):
        """Return letter choices in randomized order for this player"""
        import random
...
class WaitForPlayers(WaitPage):
    ...
    def after_all_players_arrive(self):
        # Assign treatment based on arrival order: first group gets 'info', second gets 'no_info', alternating
        if self.group.id_in_subsession % 2 == 1:
            treatment = 'info'
        else:
            treatment = 'no_info'

        # Assign same treatment to all players in this group
        for player in self.group.get_players():
            player.treatment = treatment
            player.participant.vars['treatment'] = treatment
            print(f"Player {player.id_in_group} in Group {self.group.id_in_subsession} assigned to treatment: {treatment}")
            # Initialize as active
            player.participant.vars['is_active'] = True
            player.participant.vars['timed_out'] = False

        # Create partnerships within this group
        self.group.create_initial_partnerships()
...
class Name_Game(Page):
    ...
    def is_displayed(player):
        return (not player.participant.vars.get('timed_out', False) and
                ...
                player.participant.vars.get('treatment') == 'no_info')
    ...
    def vars_for_template(player):
        ...
        return {
            ...
            'randomized_letters': get_randomized_letters()  # Fresh randomization each round
        }

class Name_Game_2(Page):
    form_model = 'player'
    form_fields = ['letter_choice']
    timeout_seconds = 120  # 2 minutes
    ...
    def is_displayed(player):
        return (not player.participant.vars.get('timed_out', False) and
                ...
                player.participant.vars.get('consent', False) == True and
                not player.participant.vars.get('attention_check_failed', False) and
                player.participant.vars.get('treatment') == 'info')
...
class MatchingTimeoutEnd(Page):
    ...
    def is_displayed(player):
        return player.participant.vars.get('matching_timeout', False)
